...
sample_background_single(len::Int = data_pt_len)::String = 
...
function sample_backgound_with_motif_single(motif::motif_type, 
    ...
    if motif_len > data_pt_len
        error("Please use a motif that has width shorter than $data_pt_len");
    end
    ...
    dna_str = join([sample_background_single(before_at), 
        ...
        sample_background_single((data_pt_len-(before_at+motif_len)))]);
...
function sample_backgound_with_motif_multiple(
    ...
    return [sample_backgound_with_motif_single(motif, len)
        for _ in 1:how_many];
end
